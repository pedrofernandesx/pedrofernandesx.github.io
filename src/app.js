...
const DEFAULT_ARTICLES = [
  {
    ...
    body: "<p>O samba-reggae nasceu nas ruas do Pelourinho como uma resposta cultural e política...</p>"
  },
  {
    id: "2",
    ...
    body: "<p>Quando a cidade dorme, o Pelourinho acorda com uma energia que resiste ao tempo...</p>"
  },
  {
    id: "3",
    ...
    body: "<p>A roda de capoeira é um espelho da sociedade baiana. Dentro dela, dois mundos coexistem em tensão...</p>"
  },
  {
    id: "4",
    ...
    body: "<p>Por trás dos trios elétricos e da alegria que contagia o mundo, existe uma batalha invisível...</p>"
  }
];
...
function loadHero() {
  ...
  // Featured: first investigação or first article
...
function loadCards() {
  ...
  if (!cat) articles = articles.slice(0, 3);
...
function loadArticle() {
  ...
  if (tagEl) tagEl.textContent = article.categoryLabel;
  if (bylineEl) bylineEl.innerHTML = `<span>${article.author}</span><span>•</span><span>⏱ ${article.readTime}</span><span>•</span><span>${formatDate(article.date)}</span>`;
